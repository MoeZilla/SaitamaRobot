...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_chat:
        ...
        msg.reply_text("Saitama AI successfully enabled for this chat!")
        message = (
        ...
        return message
    msg.reply_text("Saitama AI is already enabled for this chat!")
    return ""
...
def rem_chat(update: Update, context: CallbackContext):
    ...
    is_chat = sql.is_chat(chat.id)
    if not is_chat:
        msg.reply_text("Saitama AI isn't enabled here in the first place!")
        return ""
    sql.rem_chat(chat.id)
    msg.reply_text("Saitama AI disabled successfully!")
    message = (
        f"<b>{html.escape(chat.title)}:</b>\n"
        f"AI_DISABLED\n"
        f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
def saitama_message(context: CallbackContext, message):
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        if not saitama_message(context, message):
            return
        Message = message.text
        bot.send_chat_action(chat_id, action="typing")
        url = requests.get('https://kuki.up.railway.app/Kuki/chatbot?message='+Message)
        R = json.loads(url.text)
        r = R['reply']
        sleep(0.3)
        message.reply_text(r, timeout=60)

def list_all_chats(update: Update, context: CallbackContext):
    chats = sql.get_all_saitama_chats()
    text = "<b>SAITAMA-Enabled Chats</b>\n"
    for chat in chats:
        try:
